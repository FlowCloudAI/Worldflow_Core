...
fn row_to_entry_brief(row: &sqlx::sqlite::SqliteRow) -> Result<EntryBrief> {
    ...
    let cover = cover_str.map(|s| std::path::PathBuf::from(s));
    Ok(EntryBrief {
        id: row.try_get("id")?,
        project_id: row.try_get("project_id")?,
        category_id: row.try_get("category_id")?,
...
impl SqliteDb {
    ...
    pub async fn create_entry(&self, input: CreateEntry) -> Result<Entry> {
        ...
        let row = sqlx::query(
            "INSERT INTO entries (id, project_id, category_id, title, content, type, tags, images, cover_path)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
             RETURNING id, project_id, category_id, title, content, type, tags, images, created_at, updated_at"
        )
            .bind(&id)
    ...
    pub async fn get_entry(&self, id: &str) -> Result<Entry> {
        let row = sqlx::query(
            "SELECT id, project_id, category_id, title, content, type, tags, images, created_at, updated_at
    ...
    pub async fn list_entries(
        ...
        let rows = match category_id {
            Some(cid) => {
                sqlx::query(
                    "SELECT id, project_id, category_id, title, type, cover_path, updated_at
                    FROM entries
