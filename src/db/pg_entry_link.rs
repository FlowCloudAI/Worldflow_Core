...
fn row_to_link(row: &sqlx::postgres::PgRow) -> Result<EntryLink> {
    Ok(EntryLink {
        id: row.try_get("id")?,
...
impl EntryLinkOps for PgDb {
    ...
    async fn delete_links_from_entry(&self, entry_id: &Uuid) -> Result<u64> {
        let result = sqlx::query("DELETE FROM entry_links WHERE a_id = $1")
            .bind(entry_id)
            .execute(&self.pool)
            .await?;
    ...
    async fn replace_outgoing_links(
        ...
        sqlx::query("DELETE FROM entry_links WHERE a_id = $1")
            .bind(entry_id)
            .execute(&mut *tx)
            .await?;
